...
def process_log_data(spark, input_data, output_data):
    ...
    df = (
        df.withColumn("start_time", get_timestamp("ts"))
    ...
    song_data_df = spark.sql(
        "SELECT DISTINCT song_id, title, artist_id, artist_name, duration FROM song_data"
    )

    # join data sets
    joined_tables = df.join(
        song_data_df,
    ...
    songsplays_table.write.partitionBy("year", "month").parquet(
        os.path.join(output_data, "songsplays/songsplays.parquet"), "overwrite"
    )
